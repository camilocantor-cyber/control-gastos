...
export function FormPreviewModal({
    ...
    const handleDrop = (e: React.DragEvent, targetId: string | null) => {
        ...
        if (targetId && draggedId !== targetId) {
            ...
            // Prevent nesting accordions during reorder
            if (draggedField.type === 'accordion' && targetField.parent_accordion_id) {
                console.warn('Nesting accordions is not allowed during reorder.');
                return;
            }
                    ...
                    <div className="flex items-center justify-between px-1 min-h-[1rem] h-auto mb-1 pointer-events-none">
                        ...
                        {!isReadOnly && (
                            <span
                                ...
                                className="text-[10px] font-black text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition-all uppercase tracking-widest flex items-center gap-1 pointer-events-auto cursor-pointer py-1 px-2 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded-lg"
                            >
                    ...
                    {field.type === 'textarea' ? (
                        <textarea
                            ...
                            onChange={(e) => {
                                ...
                                setFormData(prev => ({ ...prev, [field.name]: (field.max_length && val.length > field.max_length) ? val.slice(0, field.max_length) : val }));
                            }}
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                <button
                                                    ...
                                                    className={clsx(
                                                        ...
                                                        showSaveSuccess ? "bg-emerald-500 text-white" : "bg-blue-50 dark:bg-blue-900/30 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/50"
                                                    )}
                                                    title="Guardar Control"
                                                >
                                                    {showSaveSuccess ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Save className={clsx("w-3.5 h-3.5", saving && "animate-pulse")} />}
                                                </button>
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className="p-2 bg-rose-50 dark:bg-rose-900/30 text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-900/50 rounded-lg transition-all active:scale-90"
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-lg transition-all active:scale-90"
                                                    title="Cerrar Editor"
                                                >
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    {selectedField.type === 'textarea' && (
                                                        <div className="space-y-1">
                                                            <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1 leading-none">Altura (Filas)</label>
                                                            <input
                                                                type="number"
                                                                min={2}
                                                                max={20}
                                                    ...
                                                    {(selectedField.type === 'text' || selectedField.type === 'textarea' || selectedField.type === 'email' || selectedField.type === 'phone' || selectedField.type === 'consecutivo') && (
                                                        <div className="space-y-1">
                                                            <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1 leading-none">Límite (Caracteres)</label>
                                                            <input
                                                                type="number"
                                                                min={1}
                        ...
                        <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                            ...
                            <div className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-thin">
                                                ...
                                                {selectedField.lookup_config?.mapping && Object.entries(selectedField.lookup_config.mapping).length > 0 ? (
                                                    Object.entries(selectedField.lookup_config.mapping).map(([sourceKey, targetField], idx) => (
                                                        <div key={idx} className="flex items-center gap-2 animate-in slide-in-from-left-2 duration-300" style={{ animationDelay: `${idx * 50}ms` }}>
                                                            {selectedField.lookup_config?.type === 'database' && selectedField.lookup_config?.table_name && tableColumnsMap?.[selectedField.lookup_config.table_name] ? (
                                                                <select
                                                                    value={sourceKey}
                                                                    onChange={(e) => {
                                                                        ...
                                                                        const newMapping = Object.fromEntries(
                                                                            entries.map(([k, v]) => [k === sourceKey ? newSource : k, v])
                                                                        );
                                                                        onUpdateField(selectedField.id, { lookup_config: { ...selectedField.lookup_config, mapping: newMapping } as any });
                                                                    }}
                                                                    className="flex-1 px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-[10px] font-bold shadow-sm text-slate-900 dark:text-slate-200 outline-none focus:border-blue-500"
                                                                >
                                ...
                                <div className="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                                        ...
                                        <div className="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                                            ...
                                            <div className="grid grid-cols-1 gap-6">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    {isFirstActivity && (
                                                        <label className="flex items-center justify-between w-full h-[45px] px-3 bg-indigo-50 dark:bg-indigo-900/10 border-2 border-indigo-100 dark:border-indigo-800/50 rounded-xl cursor-pointer hover:border-indigo-200 transition-all shadow-sm col-span-full">
                                                            <div className="flex flex-col">
                                                                <span className="text-[8px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest leading-none">Contexto Global</span>
                                                                <span className="text-[6px] font-bold text-slate-400 uppercase tracking-widest opacity-60">(Visible en todo el flujo)</span>
                                                            </div>
                                                            <button
                                                                onClick={() => onUpdateField(selectedField.id, { is_global_header: !selectedField.is_global_header })}
                                                                className={clsx(
                                                                    "w-8 h-4 rounded-full transition-all relative shrink-0 ml-2",
                                                                    selectedField.is_global_header ? "bg-indigo-600 shadow-md shadow-indigo-200" : "bg-slate-300 dark:bg-slate-700"
                                                                )}
                                                            >
                                                                <div className={clsx(
                                                                    "absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all",
                                                                    selectedField.is_global_header ? "left-4" : "left-0.5"
                                                                )} />
                                                            </button>
                                                        </label>
                                                    )}
                                    ...
                                    {/* Sección 5: Mapeo de Base de Datos */}
                                    {selectedField.db_column && (
                                        <div className="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800 animate-in slide-in-from-top-2">
                                            <div className="flex items-center gap-2">
                                                <Database className="w-4 h-4 text-indigo-500" />
                                                <h4 className="text-xs font-extrabold uppercase tracking-widest text-slate-800 dark:text-slate-300">Sincronización con Base de Datos</h4>
                                            </div>
                                            <div className="bg-indigo-50/50 dark:bg-indigo-900/10 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800/50 space-y-4">
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        <span className="block text-[8px] font-black text-indigo-400 dark:text-indigo-500 uppercase tracking-widest mb-1">Columna Destino</span>
                                                        <span className="text-sm font-bold text-indigo-900 dark:text-indigo-100 font-mono bg-white dark:bg-slate-900 px-2 py-1 rounded inline-block shadow-sm">
                                                            {selectedField.db_column}
                                                        </span>
                                                    </div>
                                                    <div>
                                                        <span className="block text-[8px] font-black text-indigo-400 dark:text-indigo-500 uppercase tracking-widest mb-1">Tipo de Dato SQL</span>
                                                        <span className="text-xs font-bold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 px-2 py-1 rounded inline-block shadow-sm">
                                                            {selectedField.db_type || 'Desconocido'}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div className="flex flex-wrap gap-2 pt-2 border-t border-indigo-100/50 dark:border-indigo-800/50">
                                                    {selectedField.db_is_primary_key && (
                                                        <span className="text-[9px] font-black px-2 py-1 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 rounded-lg flex items-center gap-1">
                                                            <ShieldCheck className="w-3 h-3" />
                                                            Llave Primaria (Obligatorio y Único)
                                                        </span>
                                                    )}
                                                    {!selectedField.db_nullable && !selectedField.db_is_primary_key && (
                                                        <span className="text-[9px] font-black px-2 py-1 bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-400 rounded-lg flex items-center gap-1">
                                                            <AlertCircle className="w-3 h-3" />
                                                            No Nullable (Obligatorio)
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div className="col-span-full space-y-1.5 pt-6 border-t border-slate-100 dark:border-slate-800">
