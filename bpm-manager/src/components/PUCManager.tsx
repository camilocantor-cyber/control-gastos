...
import { Plus, Edit2, Trash2, Search, ChevronRight, ChevronDown, FileText, Layout as LayoutIcon, BookMarked, PieChart } from 'lucide-react';
import type { ChartOfAccount, AccountFormData } from '../types/accounting';
import { AccountingOperationManager } from './AccountingOperationManager';
import { JournalEntryViewer } from './JournalEntryViewer';
import { BalanceReport } from './BalanceReport';

export function PUCManager() {
    const { accounts, loading, error, addAccount, updateAccount, deleteAccount, seedPUC } = useChartOfAccounts();
    const [activeTab, setActiveTab] = useState<'accounts' | 'operations' | 'journal' | 'balance'>('accounts');
    ...
    const handleInitializeOperations = async () => {
        if (confirm('¿Deseas cargar una operación contable de ejemplo (Pago a Proveedor)? Esto facilitará la configuración inicial.')) {
            await seedPUC();
        }
    };
        ...
        return childAccounts.map(account => {
            ...
            return (
                <div key={account.id} style={{ marginLeft: `${level * 24}px` }}>
                    <div className="flex items-center gap-2 p-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg group transition-colors border-b border-slate-100 dark:border-slate-800/50">
                        {hasChildren ? (
                            <button
                        ...
                        <div className="flex-1 grid grid-cols-12 gap-4 items-center">
                            ...
                            <div className="col-span-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                                {account.organization_id ? (
                                    ...
                                    <span className="text-[10px] text-slate-400 font-bold px-2 py-1 italic">Solo lectura</span>
                                )}
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className="space-y-6">
            {/* Tabs */}
            <div className="flex gap-1 bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-[1.5rem] w-fit shadow-inner">
                <button
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-700 dark:text-emerald-400 rounded-xl font-bold transition-colors border border-emerald-200 dark:border-emerald-800"
                            >
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-colors shadow-lg shadow-blue-500/20"
                            >
